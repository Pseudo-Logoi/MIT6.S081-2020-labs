...
found:
    ...
    uint64 va = KSTACK((int)(p - proc));
    if (mappages(p->pagetableKernel, va, PGSIZE, (uint64)pa, PTE_R | PTE_W) != 0) panic("allocproc -> kvminit_proc");
    p->kstack = va;

    // 设置新的上下文以开始在forkret执行，forkret返回到用户空间。
...
pagetable_t getCurrPageTable() { return myproc()->pagetableKernel; }
...
static void freeproc(struct proc* p) {
    ...
    if (p->pagetableKernel) {
        if (p->kstack) uvmunmap(p->pagetableKernel, p->kstack, 1, 1);  // 释放进程内核栈对应的物理页面
        p->kstack = 0;
