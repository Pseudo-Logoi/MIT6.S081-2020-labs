...
uint64 sys_symlink(void) {
    ...
    begin_op();
    if (argstr(0, target, MAXPATH) < 0 ||           // 获取目标路径字符串（用于计算路径长度）
        argstr(1, path, MAXPATH) < 0 ||             // 获取符号链接的路径
        argaddr(0, &p) < 0 ||                       // 获取目标路径在用户空间的地址
        (ip = create(path, T_SYMLINK, 0, 0)) == 0)  // 创建符号链接文件
    {
        end_op();
        return -1;
    }
