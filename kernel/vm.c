...
// 取消从va开始的napges页的映射（置为0），va必须页面对齐
void uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free) {
    ...
    for (a = va; a < va + npages * PGSIZE; a += PGSIZE) {
        if ((pte = walk(pagetable, a, 0)) == 0) panic("uvmunmap: walk");  // 获取最后一级的PTE
        if ((*pte & PTE_V) == 0) panic("uvmunmap: not mapped");           //
        if (PTE_FLAGS(*pte) == PTE_V) panic("uvmunmap: not a leaf");      // 只有V表示
        if (do_free) {
...
void kvmmapuser(pagetable_t kpgtable, pagetable_t upgtable, uint64 newsz, uint64 oldsz) {
    ...
    if (newsz >= PLIC) panic("kvmmapuser: newsz is larger than PLIC\n");
    ...
    for (va = oldsz; va < newsz; va += PGSIZE) {
        ...
        if (kpte == 0) { panic("kvmmapuser: no kpte\n"); }
